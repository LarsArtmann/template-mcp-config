...
class MCPValidator {
    ...
    validateEnvironmentVariables() {
        ...
        this.test('GitHub token configuration', () => {
            ...
            const hasToken = envVars.GITHUB_PERSONAL_ACCESS_TOKEN && 
        ...
        this.test('Turso configuration', () => {
            ...
            const hasUrl = envVars.TURSO_DATABASE_URL && !envVars.TURSO_DATABASE_URL.includes('your-database');
            const hasToken = envVars.TURSO_AUTH_TOKEN && !envVars.TURSO_AUTH_TOKEN.includes('your-auth-token');
        ...
        for (const server of testServers) {
            if (config.mcpServers[server.name]) {
                this.test(`Package access: ${server.name}`, () => {
                    const result = this.exec(`bunx ${server.package} --help`, { timeout: 15000 });
                    if (result === null) {
